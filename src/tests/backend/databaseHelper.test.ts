...
import {getDocumentsByFilter, getRecruiterByUserId, getTalentByUserId} from "../../helpers/queries/databaseHelper";
...
test("Recruiter file retrieved", async()=> {
    ...
    let testRecruiter = createRecruiter(userId);
    let dbAuth = testEnv.authenticatedContext(userId).firestore();
    await addRecruiterToDB(testRecruiter, userId, dbAuth)
...
test("Talent file retrieved", async()=> {
    ...
    await addRecruiterToDB(testRecruiter, recruiterUserId, dbAuth);

    const talentUserId = "KtDtaldROMaQ93TBPCTjqTNs1rK3";
    let testTalent = createTalent(talentUserId);

    await addSkillGitToDB(dbAuth, talentUserId);
    await addSpoken_lanGermanToDB(dbAuth, talentUserId);
    await addProg_lanJavaToDB(dbAuth, talentUserId);
    await addTalentUsrToDB(testTalent, talentUserId, dbAuth);
...
test("TalentDoc got by getDocumentsByFilter", async()=> {
    ...
    await addSpoken_lanGermanToDB(dbAuth, talentUserId);
    await addProg_lanJavaToDB(dbAuth, talentUserId);
    await addJob_RoleStudentToDB(dbAuth, talentUserId);
    await addCountriesSwitzerlandToDB(dbAuth, talentUserId);
    await addCantonZurichToDB(dbAuth, talentUserId);
